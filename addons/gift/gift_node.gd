...
func get_token() -> void:
	...
	OS.shell_open("https://id.twitch.tv/oauth2/authorize?response_type=code&client_id=" + client_id +"&redirect_uri=http://localhost:3000&scope=" + scope)
	server.listen(3000)
	...
	while(peer.get_status() == peer.STATUS_CONNECTED):
		...
		if (peer.get_available_bytes() > 0):
			...
			if (response == ""):
				print("Empty response. Check if your redirect URL is set to http://localhost:3000.")
				return
			...
			if (data.has("error")):
				...
				var request : HTTPRequest = HTTPRequest.new()
				add_child(request)
				request.request("https://id.twitch.tv/oauth2/token", [USER_AGENT, "Content-Type: application/x-www-form-urlencoded"], HTTPClient.METHOD_POST, "client_id=" + client_id + "&client_secret=" + client_secret + "&code=" + data["code"] + "&grant_type=authorization_code&redirect_uri=http://localhost:3000")
...
func get_chatters() -> Array:
	var request : HTTPRequest = HTTPRequest.new()
	add_child(request)
	request.request("https://api.twitch.tv/helix/chat/chatters?broadcaster_id="+user_id+"&moderator_id="+user_id, [USER_AGENT, "Authorization: Bearer " + token["access_token"], "Client-Id:" + client_id, "Content-Type: application/json"], HTTPClient.METHOD_GET, "")
	var reply : Array = await(request.request_completed)
	request.queue_free()
	var response : Dictionary = JSON.parse_string(reply[3].get_string_from_utf8())
	if (response.has("error")):
